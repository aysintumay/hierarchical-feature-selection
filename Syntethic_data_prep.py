...
def creeate_lgb_dataset_v2(phi, theta, d, t,mu,sigma, n, data_path, start, end,lags, val_ratio=0.2, autogenerated = None, MA = None):
    if autogenerated:
    ...
    dataset =make_classification(data.shape[0], n_features = 10, n_informative = 10, n_redundant=0, weights = [0.8],)
    col_names = []
    for i in range(10):
    ...
    data[dataset_y == 1] = data[dataset_y == 1] * 1.2
    data[dataset_y == 0] = data[dataset_y == 0] * 0.8
    noise = np.random.normal(0, 0.3, data.shape[0])
    ...
    X_train2, X_test2, = train_test_split(second_set, test_size=val_ratio,
                                                        random_state=42, shuffle=False)
    y_train = pd.DataFrame(y_train, index=X_train.index, columns=["y"])
    y_test = pd.DataFrame(y_test, index=X_test.index, columns=["y"])
